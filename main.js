// These variable hold the numbers we want to do operations on and the name of the operation we want
// to perform.
...
// this function takes in the number you type in the input field and saves it to the "firstNum" 
//variable
const saveFirstNumber = (num) => {
  firstNum = parseInt(num)  
}

// this function takes in the number you type in the 2nd input field and saves it to the "secondNum"
// variable
...
// These variables are already defined but that don't point to functions. It's up to you to build 
//the functions to complete your calculator use:

const multiply = (numA, numB) => {
  // * to get a product then return it
  // Open up the inspector tool in Chrome and select the Console tab to see what this functions is 
...
// This function changes the "operation" variable to be equal to the "id" of the button we choose on
// the web page.
...
const putResultInElement = (operationResults) => {
  // access the DOM by writing "document" then use the method "getElementById" and pass it the id,
  // "result".
  document.getElementById("result").innerHTML = "Results: " + operationResults

  // Remember, each element has built in properties like "innerHTML" which we can change to anything 
  //we like. 
  // Here we give it a string: "Results: " and add the value of the operation to it.
}

// The function uses the value of "operation" variable to determine which operation function it 
//should use on the number: add, subtract, multiply, divide, or modulus
const equals = () => {
  switch (operation) {
